...
function optionsframework_mlu_init () {
	register_post_type( 'optionsframework', array(
		'labels' => array(
			'name' => __( 'Theme Options Media', 'albinomouse' ),
		),
		'public' => true,
		'show_ui' => false,
...
function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
	...
	$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload', 'albinomouse' ) . '" rel="' . $int . '" />' . "\n";
	...
	if ( $value != '' ) { 
		...
		if ( $image ) {
			...
			$title = __( 'View File', 'albinomouse' );
...
function optionsframework_mlu_modify_tabs ( $tabs ) {
	$tabs['gallery'] = str_replace( __( 'Gallery', 'albinomouse' ), __( 'Previously Uploaded', 'albinomouse' ), $tabs['gallery'] );
	return $tabs;
}
